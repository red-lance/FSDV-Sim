...
Examples:
    ...
    # detection-dropout sweep, 20 episodes per point, 3x faster than realtime
    ...
    # bearing-noise sweep with a fixed FP rate
    python3 scripts/run_sweeps.py --episodes 20 \
        --sweep bearing_noise_deg=0,1,2,4,8 \
        --fixed false_positives_per_frame=0.5 --out bearing.csv

An episode PASSES when laps >= --laps, cones_hit == 0 and final_v < 0.05.
"""
...
DRIVER_PARAMS = {
    "mission", "cones_topic", "laps", "target_speed", "min_speed",
    "max_cone_range", "min_target_dist", "half_track", "cone_timeout",
    "blind_stop_time", "lap_arm_distance", "lap_close_distance",
    "accel_limit", "brake_limit", "kp", "wheelbase", "steer_limit",
...
def parse_args():
    ...
    ap.add_argument("--laps", type=int, default=3, help="laps per episode / pass bar")
    ap.add_argument("--speed", type=float, default=1.0,
                    help="realtime multiple (harness realtime_factor + driver tick_rate)")
...
def run_episode(cfg, seed, args, env):
    driver_p = {"laps": args.laps, "tick_rate": 50.0 * args.speed}
    harness_p = {"seed": seed, "realtime_factor": args.speed}
    if args.profile:
        harness_p["profile_json"] = os.path.abspath(args.profile)
    for k, v in cfg.items():
        (driver_p if k in DRIVER_PARAMS else harness_p)[k] = v
    ...
    result = None
    try:
        out, _ = harness.communicate(timeout=TIME_CAP / args.speed + 60)
...
def main():
    ...
    metric_cols = ["laps", "dev_mean", "dev_max", "cones_hit",
                   "max_v", "time", "final_v"]
    cols = sorted(set(k for c in configs for k in c)) + ["seed"] + metric_cols + ["success"]
    ...
    with open(args.out, "w", newline="") as f:
        ...
        for cfg in configs:
            ...
            for ep in range(args.episodes):
                ...
                r = run_episode(cfg, seed, args, env)
                done += 1
                ...
                print("[%d/%d] %s seed=%d -> %s   (%.0fs elapsed)"
                      % (done, total, label, seed,
                         "PASS" if row["success"] else "FAIL", time.time() - t0),
                      flush=True)
            print(">> %s: %d/%d pass" % (label, passes, args.episodes), flush=True)
